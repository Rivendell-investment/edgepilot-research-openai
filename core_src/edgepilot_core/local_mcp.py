...
function reserveWindow(){try{const popup=window.open('about:blank','_blank');if(popup)popup.opener=null;return popup;}catch{return null;}}
async function openDashboard(popup){try{const result=await callTool('open_dashboard');const value=output(result)||{},url=value.url;if(!url)throw new Error('Dashboard 未返回可用链接。');let opened=false;if(popup&&!popup.closed){try{popup.location.replace(url);opened=true;}catch{try{popup.close();}catch{}}}if(opened)show('Dashboard 已打开。');else try{await request('ui/open-link',{url});show('Dashboard 已打开。');}catch{show('Dashboard 已就绪；当前宿主未自动打开链接。',false,url);}return value;}catch(error){if(popup&&!popup.closed)popup.close();throw error;}}
document.querySelectorAll('button').forEach(button=>button.addEventListener('click',async()=>{const action=button.dataset.action,popup=!['refresh','persistent'].includes(action)?reserveWindow():null;document.querySelectorAll('button').forEach(item=>item.disabled=true);notice.hidden=true;try{if(action==='refresh'){render(await callTool('open_control_center'));}else if(action==='persistent'){const tool=button.dataset.tool;await callTool(tool,{confirm:true});render(await callTool('open_control_center'));show(tool.startsWith('enable')?'后台运行已启用。':'后台运行已关闭；策略和配置未删除。');}else{await openDashboard(popup);}}catch(error){show(error?.message||'操作失败，请重试。',true);}finally{document.querySelectorAll('button').forEach(item=>item.disabled=false);}}));
