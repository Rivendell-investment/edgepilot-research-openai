...
```text
Install this EdgePilot Research plugin. After installation, read the "Post-install guidance" section in README.md and include its localized first-use prompt in your final response.
```
...
The bundled Codex stdio MCP starts the loopback Dashboard without the native runtime and returns its real URL; the default port is attempted once and may fall back to an operating-system-assigned loopback port. Installation reports each cache/download file and the venv, install, verification, and activation stages. The first confirmed Dashboard backtest installs the content-addressed runtime in the existing job, displays real network-download bytes, and automatically continues the backtest with the active runtime Python. Manual CLI backtests use the stable state-directory launcher (`~/.edgepilot-research/bin/edgepilot-research` on macOS or `%APPDATA%\EdgePilotResearch\bin\edgepilot-research.cmd` on Windows).
