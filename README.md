...
Backtests execute the installed package's formal `strategy.py` through NautilusTrader using the Manifest V2 benchmark preset. The CLI and Dashboard allow 90-day or 365-day periods, defaulting to 90. Before execution, Research uses the preset's exact Binance Futures instrument and bar type to download public bars into an atomically replaced local catalog; it does not read credentials or initialize an execution client. The selected period is canonical `[start, end)` close time: the native Binance open-time request is shifted back one bar, then filtered so the bar closing at `start` is retained and the bar closing at `end` is excluded. When the process has standard `HTTPS_PROXY` or `https_proxy` set, Research forwards that value to the native Binance HTTP client for the current request; it does not persist the proxy or ship a default address. Unsupported venues fail explicitly. CSV data import remains available for a local file and explicit Nautilus instrument JSON, and arbitrary data URLs are never accepted.
