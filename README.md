...
The bundled Codex stdio MCP completes its protocol startup without starting the loopback Dashboard. The first `open_dashboard` call starts or reconnects to the Dashboard without the native runtime and returns its real URL; the default port is attempted once and may fall back to an operating-system-assigned loopback port. Installation reports each cache/download file and the venv, install, verification, and activation stages. The first Dashboard backtest uses an accessible in-page confirmation that states the runtime download size and install path; it never depends on a browser-native confirmation dialog. Confirming creates the existing job, installs the content-addressed runtime, displays real network-download bytes, and automatically continues the backtest with the active runtime Python. Cancelling displays an explicit notice and creates no backtest job. Manual CLI backtests use the stable state-directory launcher (`~/.edgepilot-research/bin/edgepilot-research` on macOS/Linux or `%USERPROFILE%\.edgepilot-research\bin\edgepilot-research.cmd` on Windows).
...
Runtime, strategies, catalog data and runs are isolated under `~/.edgepilot-research/`; on Windows this is `%USERPROFILE%\.edgepilot-research`, which avoids Microsoft Store/MSIX AppData virtualization and leaves room for runtime DLL paths. The installer writes `bin/edgepilot-research` and, on Windows, `bin/edgepilot-research.cmd`. After installation, run that launcher by absolute path for every command, including `runtime status`, `backtest`, and `ui`. Do not invoke the plugin with host Python or `python -m edgepilot_research`; that skips the locked venv, so the Dashboard can report a local runtime while backtests fail with `edgepilot-core is unavailable`. `runtime repair` validates state and rewrites the launcher; `runtime uninstall --yes` removes only runtime files and preserves strategies, catalog and runs. An explicit `EDGEPILOT_RESEARCH_HOME` still overrides the default for isolated development or deployment. Existing `%APPDATA%\EdgePilotResearch` or MSIX-private state is not deleted or merged automatically; reinstall the desired strategy and runtime into the new root, or use an explicit short override when a controlled migration is required. See [`../docs/windows-research-path-compatibility.md`](../docs/windows-research-path-compatibility.md).
...
When a package exposes venue alternatives for the same preset, the selector must render the benchmark venue name rather than a blank value. Internally the benchmark choice remains an omitted `venue` request field, while an explicitly selected alternative is sent by name.
...
recommendation cards followed by the Research Dashboard:
...
The new task completes the questionnaire first. After confirmation it renders the three recommendation cards and calls `open_dashboard` directly with the same `locale` used for `recommend_strategies`, so the verified URL opens the Dashboard in that language. The user does not need to send another Dashboard command. If the host blocks automatic navigation, the tool result still provides the verified clickable URL. This flow does not install or check the native runtime.
