...
When the onboarding request also asks to open the Research Dashboard, wait until `recommend_strategies` successfully renders the three cards, then call `open_dashboard` directly; the original onboarding request is explicit authorization for this read-only local action. Use only the verified URL returned by the current MCP, and do not ask the user to send another Dashboard command. If the host blocks automatic navigation, present that verified URL as the single fallback link. Opening the bundled Dashboard never checks or installs the native runtime.
